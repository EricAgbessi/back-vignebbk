...
class FiltersService {
    ...
    async getAllFilters() {
        try {
            const [types, categories, regions, domaines, classifications, cepages, styles, prixRanges, millesimes, alcoolRanges, champagneTypes, champagneStyles, cognacCategories, cognacRegions, accordsCategories,] = await Promise.all([
    ...
    async getProductTypes() {
        const types = await utils_1.prisma.produits.groupBy({
            by: ["type"],
            _count: { id: true },
            orderBy: { type: "asc" },
        });
    ...
    // Catégories
    async getCategories() {
        const categories = await utils_1.prisma.produits.groupBy({
            by: ["cat_gorie"],
    ...
    // Domaines
    async getDomaines() {
        const domaines = await utils_1.prisma.domaines.findMany({
            where: {
                produits: { some: {} },
            },
            select: {
    ...
    // Styles
    async getStyles() {
        const styles = await utils_1.prisma.produits.groupBy({
            by: ["style"],
    ...
    // Plages de prix
    async getPrixRanges() {
        const prixStats = await utils_1.prisma.produits.aggregate({
            _min: { prix: true },
            _max: { prix: true },
            _avg: { prix: true },
        });
        ...
        const rangesWithCount = await Promise.all(ranges.map(async (range) => {
    ...
    // Millésimes
    async getMillesimes() {
        const millesimes = await utils_1.prisma.produits.groupBy({
            by: ["millesime"],
    ...
    // Types de Champagne
    async getChampagneTypes() {
        const types = await utils_1.prisma.champagne_details.groupBy({
            by: ["type_champagne"],
    ...
    // Styles de Champagne
    async getChampagneStyles() {
        const styles = await utils_1.prisma.champagne_details.groupBy({
            by: ["style_champagne"],
            _count: { produit_id: true },
    ...
    async getCognacCategories() {
        const categories = await utils_1.prisma.cognac_details.groupBy({
            by: ["cat_gorie__ge"],
            _count: { produit_id: true },
    ...
    // Régions de Cognac
    async getCognacRegions() {
        const regions = await utils_1.prisma.cognac_details.groupBy({
            by: ["r_gion_cognac"],
            _count: { produit_id: true },
    ...
    async getAccordsCategories() {
        const categories = await utils_1.prisma.accords_mets.groupBy({
            by: ["cat_gorie"],
            _count: { id: true },
            orderBy: { cat_gorie: "asc" },
        });
    ...
    async getProductCount(where) {
        return await utils_1.prisma.produits.count({ where });
    }
    // Formateur de labels
    formatLabel(label) {
        if (!label)
